...
Чтобы получить список всех факультетов учебного завеления, нужно отправить `GET` запрос на адрес `http://api.studgram.ru/api/institutions/{ID_ОРГАНИЗАЦИИ}/faculties`.  
В качестве ответа вам придёт массив следующих JSON-объектов:
...
### 3.5 Прикрепление студента к учебной организации
...
### 3.6 Открепление студента от учебной организации

Для открепления студента от учебной организации вам необходимо выполнить `DELETE` запрос на адрес `https://api.studgram.ru/api/students/{ID_СТУДЕНТА}/institution`.  
При успешном выполнении запроса вы открепите студента от текущей учебной организации и факультета.
...
### 3.8 Открепление студента от факультета

Для открепления студента от учебной организации вам необходимо выполнить `DELETE` запрос на адрес `https://api.studgram.ru/api/students/{ID_СТУДЕНТА}/faculty`.  
При успешном выполнении запроса вы открепите студента от текущего факультета, но он продолжит числиться за выбранной организацией.

### 3.9 Получние информации о факультете студента
...
### 3.10 Получение ID студента по его ID в мессенджере MAX, Telegram или ВКонтакте
